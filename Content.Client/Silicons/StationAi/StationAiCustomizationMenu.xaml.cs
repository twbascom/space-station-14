...
public sealed partial class StationAiCustomizationMenu : FancyWindow
{
    ...
    public void InitializeMenu(EntityUid owner)
    {
        _owner = owner;
        ...
        _isMalf = _entManager.HasComponent<MalfAiBrainComponent>(insertedAi);
    ...
    public bool IsMalfAi()
    {
        return _isMalf;
    }

    public void ShowMalfErrorPopup()
    {
        var errorPopup = new StationAiErrorWindow();
        var random = new System.Random();
        var x = 0.15f + (float)random.NextDouble() * 0.70f;
        var y = 0.15f + (float)random.NextDouble() * 0.70f;
        errorPopup.OpenCenteredAt(new Vector2(x, y));
    }
    ...
    private sealed class StationAiCustomizationGroupContainer : BoxContainer
    {
            ...
            IPrototypeManager protoManager)
        {
            ...
            foreach (var protoId in groupPrototype.ProtoIds)
            {
                ...
                var entry = new StationAiCustomizationEntryContainer(groupPrototype, prototype, selectedPrototype, buttonGroup, menu);
                AddChild(entry);
    ...
    private sealed class StationAiCustomizationEntryContainer : BoxContainer
    {
            ...
            StationAiCustomizationMenu menu)
        {
            ...
            SelectButton.OnPressed += args =>
            {
                if (menu.IsMalfAi())
                {
                    SelectButton.Pressed = (prototype == selectedPrototype);
                }
