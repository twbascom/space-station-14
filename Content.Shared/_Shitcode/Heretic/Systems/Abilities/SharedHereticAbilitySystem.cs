...
public abstract partial class SharedHereticAbilitySystem : EntitySystem
{
    ...
    public bool TryUseAbility(EntityUid ent, BaseActionEvent args)
    {
        ...
        && !mind.MindRoleContainer.ContainedEntities.Any(q => HasComp<HereticRoleComponent>(q)))
            return false;
    ...
    public void IHateWoundMed(EntityUid uid,
        ...
        DamageableComponent? dmg = null,
        BodyComponent? body = null,
        ConsciousnessComponent? consciousness = null)
    {
        Resolve(uid, ref dmg, ref body, ref consciousness, false);
        ...
        else
        {
            ...
            _dmg.SetAllDamage((uid, dmg), 0);
            _mobThreshold.SetAllowRevives(uid, false, thresholds);
        }

        if (body != null && body.BodyType == BodyType.Complex && (boneHeal != FixedPoint2.Zero || woundHeal != FixedPoint2.Zero))
        {
            if (_body.TryGetRootPart(uid, out var rootPart, body) && rootPart != null)
            {
                foreach (var woundable in _wound.GetAllWoundableChildren(rootPart.Value))
                {
                    ...
                    var boneOpt = woundable.Comp.Bone.ContainedEntities.FirstOrNull();
                    if (boneOpt == null)
                        continue;
                    var bone = boneOpt.Value;

                    if (!TryComp<BoneComponent>(bone, out BoneComponent? boneComp))
                        continue;
        ...
        if (painHeal != FixedPoint2.Zero && painHeal is { } painHealVal)
        {
            var nerveSystem = consciousness?.NerveSystem;
            if (nerveSystem != null)
            {
                foreach (var painModifier in nerveSystem.Comp.Modifiers)
                {
                    if (painModifier.Value.Change > -painHealVal)
                    {
                        // This reduces pain maybe, who the hell knows
                        _pain.TryChangePainModifier(nerveSystem.Owner,
                            painModifier.Key.Item1,
                            painModifier.Key.Item2,
                            painModifier.Value.Change + painHealVal,
                            nerveSystem.Comp);
                        continue;
                    }

                    _pain.TryRemovePainModifier(nerveSystem.Owner,
                        painModifier.Key.Item1,
                        painModifier.Key.Item2,
                        nerveSystem.Comp);
                }

                foreach (var painMultiplier in nerveSystem.Comp.Multipliers)
                {
                    // Uhh... just fucking remove it, who cares
                    _pain.TryRemovePainMultiplier(nerveSystem.Owner,
                        painMultiplier.Key,
                        nerveSystem.Comp);
                }

                foreach (var nerve in nerveSystem.Comp.Nerves)
                {
        ...
        if (bloodHeal == FixedPoint2.Zero || !TryComp(uid, out SolutionContainerManagerComponent? sol) ||
            ...
            blood.BloodSolution.Value.Comp.Solution.Volume >= blood.BloodSolution.Value.Comp.Solution.MaxVolume)
            return;

        if (bloodHeal == null)
        {
            _blood.TryModifyBloodLevel((uid, blood),
                blood.BloodSolution.Value.Comp.Solution.MaxVolume - blood.BloodSolution.Value.Comp.Solution.Volume);
        }
        else
        {
            _blood.TryModifyBloodLevel((uid, blood),
