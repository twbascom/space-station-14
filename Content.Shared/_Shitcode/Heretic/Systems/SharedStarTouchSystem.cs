...
public sealed class SharedStarTouchSystem : EntitySystem
{
    ...
    private void UpdateBeams(EntityUid uid, StarTouchedComponent touch, ComplexJointVisualsComponent? visuals = null)
    {
        ...
        foreach (var (netEnt, _) in visuals.Data.Where(x => x.Value.Id == StarTouchBeamDataId).ToList())
        {
            if (!TryGetEntity(netEnt, out var target) || TerminatingOrDeleted(target) ||
                !_transform.InRange(target.Value, uid, touch.Range))
            {
        ...
        _status.TryRemoveStatusEffect(uid, StarTouchStatusEffect);
    }
