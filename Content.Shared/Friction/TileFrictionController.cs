        ...
        public override void UpdateBeforeSolve(bool prediction, float frameTime)
        {
            ...
            foreach (var ent in PhysicsSystem.AwakeBodies)
            {
                ...
                if (body.BodyType != BodyType.KinematicController)
                {
                    ...
                    {
                        Robust.Shared.Log.Logger.Warning($"Input mover: {ToPrettyString(uid)} in TileFrictionController is not the correct BodyType, BodyType found: {body.BodyType}, expected: KinematicController.");
                    }
                    continue;
                }
