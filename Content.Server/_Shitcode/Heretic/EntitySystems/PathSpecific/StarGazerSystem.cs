...
public sealed class StarGazerSystem : SharedStarGazerSystem
{
    ...
    private void OnGhost(GhostAttemptHandleEvent args)
    {
        ...
        {
            typeof(GhostAttemptHandleEvent)
                .GetField("<CanReturnGlobal>k__BackingField", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)
                ?.SetValue(args, false);
        }
    }
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var starGazer, out var mindContainer, out var xform))
        {
            ...
            foreach (var noob in noobs2)
            {
                ...
                var angleac = MathF.Acos(Math.Clamp(Vector2.Dot(-a, -c) / (aLen * cLen), -1f, 1f));
                var anglebc = MathF.Acos(Math.Clamp(Vector2.Dot(-b, c) / (bLen * cLen), -1f, 1f));
