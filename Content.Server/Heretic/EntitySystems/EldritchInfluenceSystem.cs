...
public sealed class EldritchInfluenceSystem : EntitySystem
{
    ...
    public static readonly Regex CommandRegex = new(@"^\.(.+)");
    ...
    private void OnExamine(Entity<EldritchInfluenceComponent> ent, ref ExaminedEvent args)
    {
        ...
        OnExamineMessage(args.Examiner, ent.Comp, ent.Comp.ExamineBaseMessage, Loc.GetString(_random.Pick(ent.Comp.HeathenExamineMessages)), session);
    }

    public void OnExamineMessage(EntityUid uid, EldritchInfluenceComponent component, string baseMessage, string message, ICommonSession session)
    {
        ...
        var effects = _random.Pick(component.PossibleExamineEffects);
    ...
    public bool CollectInfluence(Entity<EldritchInfluenceComponent> influence, Entity<HereticComponent> user, EntityUid? used = null)
    {
        ...
        || !_playerMan.TryGetSessionById(actor.PlayerSession.UserId, out var session))
            return false;
    ...
    private void OnDoAfter(Entity<EldritchInfluenceComponent> ent, ref EldritchInfluenceDoAfterEvent args)
    {
        ...
        Spawn("EldritchInfluenceSpent", Transform(args.Target.Value).Coordinates);
