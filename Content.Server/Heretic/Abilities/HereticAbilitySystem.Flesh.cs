...
public sealed partial class HereticAbilitySystem
{
    ...
    private Entity<StomachComponent, OrganComponent> ResolveStomach(Entity<FleshPassiveComponent> ent)
    {
        ...
        _solution.EnsureSolution(fleshStomach, solName, out _, 1984);
        stomach = EnsureComp<StomachComponent>(fleshStomach);
        ((dynamic) stomach).DigestionDelay = TimeSpan.FromSeconds(5);
        organ = EnsureComp<OrganComponent>(fleshStomach);
        ((dynamic) organ).Body = ent;
        Dirty(fleshStomach, organ);
        var metabolizer = EnsureComp<MetabolizerComponent>(fleshStomach);
        dynamic met = metabolizer;
        met.UpdateInterval = TimeSpan.FromMilliseconds(100);
        met.MetabolismGroups = new List<MetabolismGroupEntry> { new() { Id = "Food" }, new() { Id = "Drink" } };
        met.MetabolizerTypes = new HashSet<Robust.Shared.Prototypes.ProtoId<Content.Shared.Body.Prototypes.MetabolizerTypePrototype>> { "Vox" };
        met.SolutionOnBody = false;
        met.RemoveEmpty = true;
        met.SolutionName = solName;
        ...
        EntityUid? hostile)
    {
        ...
        if (_statusEffect.TryGetTime(uid, "KnockedDown", out var knockdownStartEnd))
        {
            ...
            if (time > TimeSpan.Zero)
                _stun.TryKnockdown(clone.Value, time, refresh: true, drop: false);
        }
